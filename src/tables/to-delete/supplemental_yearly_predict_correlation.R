...
# Save output
write.csv(thresholds, file.path(here::here(), "data-updated", "model-data", "outputs", "yearly_risk_thresholds.csv"), row.names = F)
...
all_dat2 <- all_dat %>% 
  group_by(rfmo, year, latitude, longitude, cell_id) %>% 
  summarise(.final_pred = sum(.final_pred, na.rm = T), 
            catch = sum(catch, na.rm = T)) %>% 
  ungroup() %>% 
...
# Save output
write.csv(all_dat2, file.path(here::here(), "data-updated", "model-data", "outputs", "predicted_catch_yearly_risk.csv"), row.names = F)

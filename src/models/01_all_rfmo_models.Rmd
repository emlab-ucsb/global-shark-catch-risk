...
# IOTC - 5x5 with count and mt, effort reported with bycatch by flag
all_rfmo_other_models(data = mt_5x5_obs, rfmos = "IOTC", effort_source = "effort reported with bycatch (flag)", 
                           save_loc = save_loc)
# WCPFC - 1x1 with count only, effort reported with bycatch (no flag available)
all_rfmo_other_models(data = count_1x1_obs, rfmos = "WCPFC", effort_source = "effort reported with bycatch (flag)", 
                           save_loc = save_loc)
```
...
# ICCAT - 1x1 count only | mean_sst, mean_chla, mean_ssh, cv_sst, cv_chla, cv_ssh | effort reported with target catch (flag)
iccat_effort_variables <- paste0(colnames(count_1x1_obs)[grepl("target_effort_", colnames(count_1x1_obs))], collapse = "|")

all_rfmo_untuned_models(data = count_1x1_obs, rfmos = "ICCAT",
                      effort_source = "effort reported with target catch (flag)",
                      classification_variables =
                        paste0("species_commonname|sdm|mean_sst|mean_chla|mean_ssh|cv_sst|cv_chla|cv_ssh|",
                                                        iccat_effort_variables),
                      regression_variables =
                        paste0("species_commonname|sdm|mean_sst|mean_chla|mean_ssh|cv_sst|cv_chla|cv_ssh|",
                                                        iccat_effort_variables),
...
# IOTC - 5x5 count and mt| mean_sst, mean_chla, cv_sst, cv_chla | effort reported with bycatch (flag)
iotc_effort_variables <- paste0(colnames(mt_5x5_obs)[grepl("bycatch_total_effort_", colnames(mt_5x5_obs))], collapse = "|")
...
# WCPFC - 1x1 count only | mean_sst, mean_cv, mean_ssh, cv_sst, cv_chla, cv_ssh, group price | effort reported with bycatch (flag) [note that bycatch effort by flag is not reported for wcpfc]
all_rfmo_untuned_models(data = count_1x1_obs,
                rfmos = "WCPFC",
                effort_source = "effort reported with bycatch (flag)",
                classification_variables = "species_commonname|sdm|mean_sst|mean_chla|mean_ssh|cv_sst|cv_chla|cv_ssh|median_price_group|bycatch_total_effort",
                regression_variables = "species_commonname|sdm|mean_sst|mean_chla|mean_ssh|cv_sst|cv_chla|cv_ssh|median_price_group|bycatch_total_effort",
                save_loc = save_loc,
                mtry_class = 5, min_n_class = 10, mtry_reg = 5, min_n_reg = 5)

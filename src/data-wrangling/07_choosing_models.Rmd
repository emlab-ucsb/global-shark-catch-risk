...
This script tests the difference in spatial distribution data (5x5 or 1x1 cells) and catch reporting (count or count and metric tonnes converted t ocount) among the RFMOs to determine which model might be the most appropriate.
...
```{r fig.cap = "RFMO data breakdowns."}
...
```{r fig.cap = "IATTC data breakdown.",  fig.width = 5}
ggplot() + 
  geom_bar(data = short_data_table %>% 
             filter(rfmo == "IATTC") %>% 
             mutate(group = paste0(spatial_notes, " (", catch_units, ")")), mapping = aes(x = rfmo, weight = perc_total, fill = group)) + 
...
```{r fig.cap = "ICCAT data breakdown.", fig.width = 5}
ggplot() + 
  geom_bar(data = short_data_table %>% 
             filter(rfmo == "ICCAT") %>% 
             mutate(group = paste0(spatial_notes, " (", catch_units, ")")), mapping = aes(x = rfmo, weight = perc_total, fill = group)) + 
...
```{r fig.cap = "IOTC data breakdown.", fig.width = 5}
ggplot() + 
  geom_bar(data = short_data_table %>% 
             filter(rfmo == "IOTC") %>% 
             mutate(group = paste0(spatial_notes, " (", catch_units, ")")), mapping = aes(x = rfmo, weight = perc_total, fill = group)) + 
...
```{r fig.cap = "WCPFC data breakdown.", fig.width = 5}
ggplot() + 
  geom_bar(data = short_data_table %>% 
             filter(rfmo == "WCPFC") %>% 
             mutate(group = paste0(spatial_notes, " (", catch_units, ")")), mapping = aes(x = rfmo, weight = perc_total, fill = group)) + 

...
dat <- dat %>% 
  mutate(species_sciname = case_when(
    ...
    species_commonname == "MACKERELS INDIAN, NEI" ~ "SCOMBRIDAE", 
...
# Remove species that are not relevant to us (we only want sharks, tunas, and tuna-like species)
dat <- dat %>% 
...
dat <- dat %>% 
  ...
  mutate(effort_units = case_when(
    effort_units %in% c("number of sets", "number of sets |successful number of sets") ~ "sets",
    grepl("hours fished|hours fishing", effort_units) ~ "fishing hours", 
    grepl("days fished|days fishing|successful fishing days", effort_units) ~ "fishing days", 

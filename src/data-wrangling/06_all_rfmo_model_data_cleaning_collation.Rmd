...
```{r include=FALSE}
# Load Data
all_data <- vroom::vroom("../../data/rfmo-observer-data/outputs/all_data.csv") 
...
short_data <- short_data %>% 
  ...
  group_by(rfmo, year, latitude, longitude, gear_group, species_commonname, spatial_notes,
...
short_data <- short_data %>%
  ...
  pivot_wider(names_from = c(year, species_sciname, catch_units),
              ...
              values_from = catch,
              values_fill = 0) %>%
  pivot_longer(cols = colnames(.)[!grepl("latitude|longitude|rfmo|gear_group|spatial_notes", colnames(.))],
...
```{r}
# Update mt to count
mt_count <- read.csv("../../data/species-information/spp_weight_comparison.csv") %>% 
  mutate(scientific_name = str_to_upper(scientific_name)) 

# Add sharks nei to the mix
mt_count <- mt_count %>% 
  bind_rows(data.frame(scientific_name = "OTHER SHARKS", 
...
combined_data <- short_data %>%
  ...
  filter(year >= 2012) %>% # lower limit for GFW data
  left_join(all_sdm) %>% 
  left_join(gfw_data) %>% 
  left_join(sst_data) %>% 
  left_join(chla_data) %>% 
  filter(!is.na(mean_chla) | !is.na(mean_sst) | !is.na(sdm) | !is.na(total_fishing_kwh))

write.csv(combined_data, 
          "../../data/model-data/inputs/all-rfmo-models/global_ll_data_1x1.csv", 
          row.names = FALSE)
```

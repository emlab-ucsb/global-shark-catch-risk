...
species_list <- read.csv(file.path(here::here(), "data/species-information/species_list_for_sdms.csv"))

### 
# Set Values
###
spatial_scale <- 1 # adjust depending on the scale we're aiming for (5x5 cell or 1x1 cell)
```

# IUCN SDMs
```{r}
# Load, clean, and filter SDM file names
sdm_list <- list.files(file.path(here::here(), "data/iucn-sdm-data/intermediates"))
...
for(file in sdm_list) {
  temp_sdm <- read_csv(file.path(here::here(), "data/iucn-sdm-data/intermediates", file), n_max = 1)
...
for(nei in unique(species_list$species_sciname)) {
  ...
  if(length(temp_sdm_list) != 0) {
    ...
    temp_sdm <- read_csv(file.path(here::here(),"data/iucn-sdm-data/intermediates", file))
...
write.csv(all_sdm, file.path(here::here(), paste0("data/iucn-sdm-data/outputs/global_combined_iucn_sdm_",spatial_scale,"x",spatial_scale,".csv")),
          row.names = F)
```

# Sea Surface Temperature
```{r}
file_list <- list.files(file.path(here::here(), "data/sea-surface-temperature/intermediates",
                        pattern = "csv"))

sst_data <- NULL

for(file in file_list) {
  temp <- read.csv(file.path(here::here(), "data/sea-surface-temperature/intermediates", file))
  temp_save <- NULL
...
write.csv(sst_data, file.path(here::here(), paste0("data/sea-surface-temperature/outputs/binned_global_sst_",spatial_scale,"x",spatial_scale,".csv")),
          row.names = F)
```

# Chlorophyll-A
```{r}
file_list <- list.files(file.path(here::here(), "data/chlorophyll-a/intermediates"),
                        pattern = "csv")

chla_data <- NULL

for(file in file_list) {
  temp <- read.csv(file.path(here::here(), "data/chlorophyll-a/intermediates",
                             file))
  temp_save <- NULL
...
write.csv(chla_data, file.path(here::here(), paste0("data/chlorophyll-a/outputs/binned_global_chla_",spatial_scale,"x",spatial_scale,".csv")),
          row.names = F)
```

# Sea Surface Height
```{r}
file_list <- list.files(file.path(here::here(), "data/sea-surface-height/intermediates"),
                        pattern = "csv")

ssh_data <- NULL

for(file in file_list) {
  temp <- read.csv(file.path(here::here(), "data/sea-surface-height/intermediates",
                             file))
  temp_save <- NULL
...
write.csv(ssh_data, file.path(here::here(), paste0("data/sea-surface-height/outputs/binned_global_ssh_",spatial_scale,"x",spatial_scale,".csv")),
          row.names = F)
```

# Global Fishing Watch
```{r}
gfw_data <- read.csv(file.path(here::here(), "data/global-fishing-watch/inputs/1x1_gfw_query_v20190502.csv"))

tonnage_groups <- read.csv(file.path(here::here(), "data/global-fishing-watch/inputs/tonnage_groups_v20200410.csv"))
...
gfw_shift <- gfw_shift %>%
  ...
  distinct_all()
# save
write.csv(gfw_shift,  file.path(here::here(), paste0("data/global-fishing-watch/outputs/binned_global_gfw_",spatial_scale,"x",spatial_scale,".csv")), row.names = F)
```
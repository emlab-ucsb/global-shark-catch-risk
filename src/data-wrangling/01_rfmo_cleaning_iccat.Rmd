...
# Not going to use for this iteration since its L/F
# # Load t2ce size
...
# Load t2ce by school
t2ce_school <- read_xlsx(paste0(file_path, "t2ce_ETRO-PS1991-20_bySchool.xlsx"), sheet="dsT2CE_PS-ETRO_91-20", skip=6)
```
...
t2ce_school <- t2ce_school %>% 
  mutate(rfmo = "ICCAT", 
         ...
         quality_code = NA, 
         was_generated = "no", 
         sources = "t2ce_ETRO-PS1991-20_bySchool"
         )

# Keep only relevant columns
t2ce_school <- t2ce_school[,c("rfmo", "year", "month", "time_period", "country", "gear", "settype", "longitude", "latitude", "effort", "effort_units", "species_commonname", "species_sciname", "catch", "catch_units", "spatial_notes", "quality_code", "was_generated", "sources")]
...
t2ce_school  <- t2ce_school %>% 
  mutate(effort_units = gsub("Number of ", "", effort_units), 
         effort_units = gsub("made", "", effort_units), 
         effort_units = gsub("sets", "number of sets", effort_units), 
         effort_units = str_squish(effort_units)
         )

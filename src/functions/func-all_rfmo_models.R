...
all_rfmo_models <- function(data, rfmos, effort_source, save_loc){
  ...
  if(nrow(prep_ll) > 0) { 
    ...
    if(nrow(train_data_class_orig) > 0 & 
      ...
      # 2) self-reported effort associated with bycatch
      # 3) effort reported by gfw
      
      model_runs <- data.frame(model = c("model 1", "model 2", "model 3"), 
                               parameters = c("effort reported with target catch (flag)", 
                                              "effort reported with bycatch (flag)", "gfw effort"))
      ...
      if(models == "model 1") { # effort reported with target catch (flag)
        ...
        final_test <- test_data_class_orig %>% 
          dplyr::select(rfmo, latitude, longitude, spatial_notes, year, pres_abs, catch, sdm, species_commonname,
                        mean_sst, mean_chla,
                        colnames(train_data_class_orig)[grepl("target_effort_", colnames(train_data_class_orig))]) %>% 
          distinct_all() 
        ...
        full_predict_df <- prep_ll %>% 
          dplyr::select(rfmo, latitude, longitude, spatial_notes, year, pres_abs, catch, sdm, species_commonname,
                        mean_sst, mean_chla,
                        colnames(prep_ll)[grepl("target_effort_", colnames(prep_ll))]) %>% 
          distinct_all() 
      }
      ...
      if(models == "model 2") { # effort reported with bycatch (flag)
        ...
        full_predict_df <- prep_ll %>% 
          dplyr::select(rfmo, latitude, longitude, spatial_notes, year, pres_abs, catch, sdm, species_commonname,
                        mean_sst, mean_chla, 
                        colnames(prep_ll)[grepl("bycatch_total_effort_", colnames(prep_ll))]) %>% 
          distinct_all() 
      }
      ...
      if(models == "model 3") { # gfw_effort
        ...
        final_test <- test_data_class_orig %>% 
          dplyr::select(rfmo, latitude, longitude, spatial_notes, year, pres_abs, catch, sdm, species_commonname,
                        mean_sst, mean_chla, total_fishing_kwh) %>% 
          distinct_all()

...
all_rfmo_untuned_models <- function(data, save_loc, rfmos, effort_source, logtrans = FALSE, classification_variables, regression_variables, mtry_class, mtry_reg, min_n_class, min_n_reg){
  ...
  if(nrow(prep_ll) > 0) {
    ...
    prep_n <- prep_ll %>% select(-effort_units) %>% 
    ...
    for(i in 1:nrow(prep_n)) { 
      prep_temp <- prep_new %>% 
        filter(latitude == prep_n$latitude[i] & longitude == prep_n$longitude[i])
      for(j in colnames(prep_sds)[grepl("sst|chla|ssh|effort|kwh", colnames(prep_sds))]) { 
        t <- rnorm(n = prep_n$n[i], mean = prep_means[[j]][i], sd = prep_sds[[j]][i])
        prep_temp <- prep_temp %>% 
    ...
    prep_pred <- prep_ll %>% 
      select(-effort_units, -matches("sst|chla|ssh|effort|kwh")) %>% 
    ...
    saveRDS(output_fit, paste0(save_loc, rfmos, "_ll_untuned_model_test.rds"))
    write.csv(final_predict, paste0(save_loc, rfmos, "_ll_untuned_final_predict_test.csv"), row.names = F)

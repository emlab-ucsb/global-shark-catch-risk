...
all_rfmo_selector_models <- function(data_gfw, data_effort, save_loc){
  ...
  for(rfmos in unique(c(data_gfw$rfmo, data_effort$rfmo))) { 
    ...
    for(effort_source in c("effort reported with target catch (flag)", "effort reported with bycatch (flag)", "gfw effort")) { 
      ...
      if(nrow(prep_ll) > 0) { 
        ...
        if(nrow(train_data_class_orig) > 0 & 
          ...
          train_class <- train_data_class_orig %>% 
            select(-catch, -rfmo, -year, -spatial_notes, -gear_group, -species_group, -species_sciname, -catch_units,
                   -species_resolution, -zone, -location_cluster)
          
          train_reg <- train_data_reg_orig %>% 
            select(-pres_abs, -rfmo, -year, -spatial_notes, -gear_group, -species_group, -species_sciname, 
                   -catch_units, -species_resolution,  -zone, -location_cluster)
          ...
          class_recipe <- recipe(pres_abs ~ ., data = train_class) %>%
            ...
            recipeselectors::step_select_forests(all_predictors(), outcome = "pres_abs", threshold = 0.75) %>% 
          ...
          reg_recipe <- recipe(catch ~ ., data = train_reg) %>%
            ...
            step_impute_knn(all_numeric(), -all_outcomes(), neighbors = 3, impute_with = imp_vars(latitude, longitude)) %>%
            update_role(latitude, longitude, new_role = "none") %>% 
            recipeselectors::step_select_forests(all_predictors(), outcome = "catch", threshold = 0.75) %>% 
            step_dummy(all_nominal(), -all_outcomes()) 
          ...
          # See which feature are kept
          prepped_class <- prep(class_recipe)
          removed_class <- tidy(prepped_class, number = 7)
          names_class <- colnames(train_class)[!(colnames(train_class) %in% removed_class$terms)]
          ...
          # See which feature are kept
          prepped_reg <- prep(reg_recipe)
          removed_reg <- tidy(prepped_reg, number = 7)
          names_reg <- colnames(train_reg)[!(colnames(train_reg) %in% removed_reg$terms)]
          ...
          final_metrics <- final_metrics %>% 
            ...
            mutate(model = models, 
                   ...
                   predictors_class = paste(names_class, collapse = ", "), 
                   predictors_reg = paste(names_reg, collapse = ", "),
                   rfmo = rfmos) %>% 
      ...
      #   mutate_at(vars(mean_sst:colnames(.)[ncol(.)]), mean, na.rm = T) %>% 
      #   mutate_at(vars(mean_sst:colnames(.)[ncol(.)]), as.integer) %>% 
      #   distinct_all()
      # 
      # full_predict_class <- predict(class_fit, full_predict_df) %>% 
      #   mutate(.pred_class = ifelse(.pred_class == "absent", 0, 1)) %>% 
      #   bind_cols(full_predict_df)
      # 
      # full_predict_reg <- predict(reg_fit, full_predict_df) %>% 
      #   bind_cols(full_predict_df)
      # 
      # full_predict <- full_predict_class %>% 
      #   left_join(full_predict_reg) %>% 
      #   mutate(.final_pred = .pred_class*.pred) %>% 
      #   relocate(.pred, .before = .final_pred) %>% 
      #   relocate(.pred_class, .before =.pred)
      # 
      # full_metrics <- metrics(truth = catch, estimate = .final_pred, data = full_predict)
      # 
  ...
  #                    "class_predictors" = names_class,
  #                    "reg_fit" = reg_fit, 
  #                    "reg_predictors" = names_reg,
  ...
  # saveRDS(output_fit, paste0(save_loc, rfmos, "_ll_final_model.rds"))
  
  # write.csv(full_predict, paste0(save_loc, rfmos, "_ll_final_predict.csv"), row.names = FALSE)

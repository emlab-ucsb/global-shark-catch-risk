## Run RFMO specific model scenario and tune
...
all_rfmo_tuned_models <- function(data, save_loc, rfmos, effort_source, classification_variables, regression_variables, mtry_class, mtry_reg, min_n_class, min_n_reg){
  ...
  if(nrow(prep_ll) > 0) {
    ...
    # Use this to narrow down the search a little more
    class_grid <- grid_regular(
      min_n(range = c(max(best_tune_class$min_n-10, 1), best_tune_class$min_n+10)),
      mtry(range = c(max(best_tune_class$mtry-10, 2), best_tune_class$mtry+10)),
      levels = 10
    )
    ...
    reg_grid <- grid_regular(
      min_n(range = c(max(best_tune_reg$min_n-10, 1), best_tune_reg$min_n+10)),
      mtry(range = c(max(best_tune_reg$mtry-10, 2), best_tune_reg$mtry+10)),
      levels = 10
    )
    ...
    pred_class <- predict(class_tuned, prep_pred) %>% 
      mutate(.pred_class = ifelse(.pred_class == "absent", 0, 1)) %>% 
      bind_cols(prep_pred)
    
    pred_reg <- predict(reg_tuned, prep_pred) %>% 
      bind_cols(prep_pred)
    ...
    saveRDS(output_fit, paste0(save_loc, rfmos, "_ll_tuned_model.rds"))
    write.csv(final_predict, paste0(save_loc, rfmos, "_ll_final_predict.csv"), row.names = F)
